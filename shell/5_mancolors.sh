...
[[ $- == *i* ]] && b6color(){
    ...
    echo -n "$((16#$(MANPAGER=cat /usr/bin/man -w "$@" 2>/dev/null | grep -Poh "(?<=/)[^/]+\.[^\\\.]+(?=\.gz\$)" | md5sum | head -c 4)/2))">/tmp/mancolorseed$USER
    if ((USESECTION==1)); then
        local sec="$(MANPAGER=cat /usr/bin/man -w "$@" 2>/dev/null | grep -Pohm 1 '\d(?=\w*?.gz)')"
